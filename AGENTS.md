...
- Non-trivial work updates crate-local docs/plans in `crates/<crate>/docs/`.
...
- Prefer writing implementation plans in the owning crate's docs directory (for example `crates/<crate>/docs/plans/<ticket-id>-<slug>.md`).
- Keep plans short and actionable: scope, decisions, risks, and validation steps.
...
- If no ADR is needed, state why in the ticket notes/PR summary so decision intent is still explicit.

# Issues / Tracking / Plans

This project uses [Beads](https://github.com/gastownhall/beads) (`bd`) for
shared issue tracking. Issue IDs use the `cac-*` prefix. The authoritative
...
When planning multi-step work, create a Beads issue for each distinct unit and
link dependencies before implementation. The issue graph is the source of
truth for execution order. Longer prose plans live in the owning crate's
`docs/plans/` directory and reference the owning issue ID.
